...
export async function GET(request: Request) {
  try {
      ...
      const csvRows = [
        ...
        ...entries.map((entry) => {
          ...
          return [
            ...
            entry.currency,
            entry.type === "invoice_payment" ? "Invoice Payment" : "Manual Entry",
          ];
        }),
        [],
        ["Total", "", "", total.toFixed(2), profile?.currency || "USD", ""],
      ];
    ...
    if (format === "pdf") {
        ...
        <Document>
          <Page size="A4" style={styles.page}>
            ...
            <View style={styles.header}>
              <Text style={styles.title}>Annual Income Summary {year}</Text>
            ...
            {byClient.length > 0 && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Income by Client</Text>
                <View style={styles.table}>
                  ...
                  {byClient.map((client, idx) => (
                    <View key={idx} style={styles.tableRow}>
                      <Text style={styles.col1}>{client.client_name}</Text>
                      <Text style={styles.col2}>{client.count}</Text>
      ...
      pdfDocument.updateContainer(<MyDocument />);
